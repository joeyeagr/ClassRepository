...
calculateArea(x: 2.4, y: 3.9)
calculateArea(x: 4, y: 9)
...
func add (x: Int?, y: Int?) -> Int? {
    guard let myX = x, let myY = y else {
        return nil
    }
    return myX + myY
}
 ...
 */
let someUser = createUser()
if let tempUser = someUser {
    print("\(tempUser.firstName) \(tempUser.lastName) is \(tempUser.age) years old")
}
